...
public class UserCrawler implements Runnable {
	...
	// 停止任务标志
	private static AtomicBoolean stop;//数据类型AtomicBoolean:使用 AtomicBoolean 高效并发处理 “只初始化一次” 的功能要求
	// 当前爬虫的id
	private int id;
	// 用户缓存
	...
	static {
		stop = new AtomicBoolean(false); //终止标志初始化为否
		try {
			// 登录一次即可
			login();
			// 保存数据线程先启动
	...
	private List<String> crawUser(String userId, String tag) throws IOException {
		...
		urlBuilder.append("/u/").append(userId).append(tag);//查看粉丝的网页http://home.cnblogs.com/u/wentingtu/followers/
		// 请求页面
	...
	private static void login() throws UnsupportedEncodingException,
		...
		Document doc = Jsoup.parse(page);
		// 取登录表格
		Element loginform = doc.getElementById("frmLogin");//<form name="frmLogin"></form>标签，内部包含了下面的内容
		Elements inputElements = loginform.getElementsByTag("input");
		List<String> paramList = new ArrayList<String>();
		for (Element inputElement : inputElements) {
			String key = inputElement.attr("name");
			String value = inputElement.attr("value");
			if (key.equals("tbUserName")) { //<input name="tbPassword" ></input>
				value = "golden1314521@gmail.com";//Test.Name;
			}
			else if (key.equals("tbPassword"))
				value = "code";//Test.passwd;
	...
	private static String sendPost(String url, String postParams)
		...
		//if(cookies == null) System.out.println("cookies is null");
		if (co != null)
			for (String c : co) {
